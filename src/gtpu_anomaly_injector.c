...
void *sender_thread(void *arg) {
  ...
  const long nanos_per_pkt = 1000000000L / pps;
  ...
  while (!stop) {
    ...
    int inner_len = build_inner_packet(inner_buf, src_ip, dst_ip,
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "", long_opts, NULL)) != -1) {
    ...
    case 'd':
      ...
      break;
    case 't':
      parse_list(optarg, &teids, NULL, &num_slices, 1);
      break;
    case 'q':
      parse_list(optarg, NULL, &qfis, &num_slices, 0);
      break;
    case 'p':
      pps = atoi(optarg);
      break;
    ...
    default:
      fprintf(stderr,
              "Usage: %s --interface IF --src-ip IP --dst-ip IP --teids LIST "
              "--qfis LIST\n",
              argv[0]);
      exit(1);
    }
  ...
  printf("[+] Sent %lu packets (%.2f Mbps) in %d seconds using %d threads\n",
